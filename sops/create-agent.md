...
- Whether a human must approve before the agent acts
...
4. Fill `channel-agent.yaml`: `name`, `channel`, `provider`, `baseAgentType`. Leave `agent.id` blank
   until created.
...
## Human-in-the-loop
Step 8 (approval before provisioning) is mandatory.
